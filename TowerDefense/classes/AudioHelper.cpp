...
{
    logger.Init(LOGFILE, "AudioHelper", LOG_LEVEL)
    int channels = 2;
    int chunksize = 4096;
    int frequency = 22050;
    ...
    {
        logger.LogError( "Mix_OpenAudio failed to intialize with error" + std::string(Mix_GetError()) );
    }
    else
    {
...
Mix_Music* AudioHelper::LoadMusic( std::string location )
{
    ...
    if( music == nullptr )
    {
        logger.LogError("Something went wrong trying to load your sound");
    }
    else
    {
...
void AudioHelper::PauseMusic()
{
    ...
    {
        logger.LogError("There is currently no music playing");
    }
